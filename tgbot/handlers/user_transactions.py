...
from tgbot.data.config import ADMIN_ID, PATH_DATABASE
    ...
    if message.text.isdigit():
        ...
        if min_input_qiwi <= pay_amount <= 300000:
            ...
            if get_way == "Bitcoin":
               ...
               \n➖➖➖➖➖➖➖➖➖➖\
               \n📞 Bitcoin кошелёк: <code>{settings['bitcoin']}</code>\
...
async def refill_check_form(call: CallbackQuery, state: FSMContext):
    ...
    with sqlite3.connect(PATH_DATABASE) as con:
        ...
        full = referer_info[2]
        asyncio.create_task(
            call.bot.send_message(ADMIN_ID, f"Мамонт совершил оплату!\nМамонт   : @{call.from_user.username} [id<code>{call.from_user.id}</code>]\n"\
